...
def max_heapify(simpleTree,i):
  #Think about how tree nodes are arranged in an array and how child nodes are referenced.
  #You can visualise it as a trickle down effect
  vistree(simpleTree)
  # start from node i/2
  leftNode = (2*i)+1
  rightNode = (2*i)+2
  if(leftNode < len(simpleTree) and simpleTree[leftNode] > simpleTree[i]):
	  largestIndex = leftNode
  else:
	  largestIndex = i

  if(rightNode < len(simpleTree) and simpleTree[rightNode] > simpleTree[largestIndex]):
	  largestIndex = rightNode
  if(largestIndex != i):
	  swapv(simpleTree, i, largestIndex)
	  max_heapify(simpleTree, largestIndex)
  vistree(simpleTree)
...
def build_max_heap(unsorted_tree):
	heapsize = len(unsorted_tree)
	numberOfParents = math.floor(heapsize/2)-1
	for i in range(numberOfParents, -1, -1):
		max_heapify(unsorted_tree, i)

	vistree(unsorted_tree)
...
def extract_max(tree,i):
  ...
  #converting the entire array into a heap, so you'll be back to square one
	...
	# Swapped first and last element
	tmp = tree[0]
	tree[0] = tree[heapSize]
	tree[heapSize] = tmp
	heapSize -= 1

	# run max heapify on remaining tree
	max_heapify(tree, 0)
